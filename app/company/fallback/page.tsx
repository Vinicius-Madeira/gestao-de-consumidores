...
export default function OfflineTemplate() {
  ...
  // Handle offline navigation for non-existent companies
  useEffect(() => {
    if (!isOnline && companyError && !companyLoading) {
      // If offline and company not found, redirect to company list
      router.push("/company");
    }
  }, [isOnline, companyError, companyLoading, router]);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          {!isOnline && (
            <p className="mt-1 text-xs text-orange-600">Modo offline</p>
          )}
        </div>
      </div>
    );
        ...
        <div className="text-center">
          ...
          {!isOnline && (
            <p className="text-sm text-orange-600 mt-2">
              Você está offline. Esta empresa pode não estar em cache.
            </p>
          )}
          <button
